...
router.post('/add',checkAuth,function(req,res){
    ...
    post.save(function(err,rtn){
        if(err){
            ...
            res.status(201).json({message:'post add success',post:rtn})
        }
    })
});
    ...
    for(var ops of req.body){
     ...
     Post.findByIdAndUpdate(req.params.id,{$set:updateOps},function(err,rtn){
         if(err){
             ...
             res.status(200).json({message:"update success",post:rtn})
         }
     })
 });
